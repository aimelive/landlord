...
function isSelected(month: string): boolean {
  ...
  return (props.selectedPayment.coveredMonths as readonly string[]).includes(
    month,
  );
}
...
<template>
  <div
    class="bg-white rounded-2xl p-5 mb-7 shadow-sm border"
  ...
  >
    ...
    <div
      v-else
      class="grid gap-2.5 grid-cols-[repeat(auto-fill,minmax(130px,1fr))]"
    >
      <div
        v-for="month in filteredMonths"
        :key="month"
        class="rounded-xl p-3 cursor-pointer transition-all outline-none border-2 hover:-translate-y-px hover:shadow-sm focus-visible:ring-2 focus-visible:ring-offset-2"
        :style="
          isSelected(month)
      ...
      >
        ...
        <div
          ...
          style="
            background: rgba(41, 159, 77, 0.1);
            color: #299f4d;
            border-color: rgba(41, 159, 77, 0.3);
          "
        >
